...
async def editar_obra(update: Updater, context: ContextTypes.DEFAULT_TYPE):
    if Conta.admin(update.message.chat.id):
        try:
            txt = update.message.text.split("editarobra ")[1].split("|")
            conteudo = txt[0]
            tipo = int(txt[1])
            obra_id = txt[2]
            Obra.editar_obra(obra_id, tipo, conteudo)
